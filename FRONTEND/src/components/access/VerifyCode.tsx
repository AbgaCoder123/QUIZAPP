...
function VerifyCode() {
  ...
  async function Verifycode() {
    const response = await fetch(
      `${domain}/api/access/reset_password/confirm_code`,
      {
        method: "post",
        body: JSON.stringify({
          code: Inputs.code,
          token: localStorage.resetToken,
        }),
        headers: {
    ...
    if (response.ok) {
      alert(result);
      window.location.href = "/access/createnewpassword";
    } else {
      alert(result);
    }
  }
    ...
    <MainContainer addtional_class="flex items-center overflow-hidden relative">
      ...
      <Container addtional_class="gap-5 flex flex-col items-center justify-center">
        <Accesstitle
          text={
            <p>
              Verify Code in <span className="primary-text">Escroll</span>
            </p>
          }
        />
        <form
